...
Check if visual verification is enabled and applicable to this phase.

**Load config:**
```bash
VISUAL_ENABLED=$(cat .planning/config.json 2>/dev/null | grep -o '"visual_verification"' && cat .planning/config.json | grep -o '"enabled"[[:space:]]*:[[:space:]]*true' | head -1 && echo "yes" || echo "no")
```

**If `VISUAL_ENABLED=no`:** Skip to verify_phase_goal.

**Check for visual verifications in completed plans:**
```bash
# Look for visual_verifications sections in this phase's PLANs
VISUAL_PLANS=$(grep -l "<visual_verifications>" "$PHASE_DIR"/*-PLAN.md 2>/dev/null)
```

**If no visual_verifications sections found:** Skip to verify_phase_goal.

**For each plan with visual verifications:**

1. **Create verification directory:**
   ```bash
   mkdir -p "$PHASE_DIR/verification/screenshots"
   ```

2. **Read visual config and secrets:**
   ```bash
   CONFIG_CONTENT=$(cat .planning/config.json)
   SECRETS_CONTENT=$(cat .planning/.secrets 2>/dev/null || echo "")
   ```

3. **Extract visual_verifications from plan:**
   ```bash
   # Extract content between <visual_verifications> tags
   sed -n '/<visual_verifications>/,/<\/visual_verifications>/p' "$PLAN_PATH"
   ```

4. **Spawn visual verifier agent:**
   ```
   Task(
     prompt="Verify visual elements for phase {phase_number}.
   ...
   {extracted visual_verifications content}

   ## Output Location
   Save report to: {PHASE_DIR}/verification/REPORT.md
   Save screenshots to: {PHASE_DIR}/verification/screenshots/

   Execute verifications and return status.",
     subagent_type="gsd-visual-verifier",
     model="{verifier_model}"
   )
   ```

5. **Handle visual verification result:**

   | Result | Action |
   |--------|--------|
   | `PASSED` | Delete screenshots folder, continue to verify_phase_goal |
   | `PARTIAL` | Keep report, add to pending_issues, continue |
   | `FAILED` | Route to retry loop |

**Retry loop for FAILED verifications:**

Track retry count per plan (max from config, default 2).

```
retry_count = 0
max_retries = config.visual_verification.max_retries || 2

while status == FAILED and retry_count < max_retries:
    1. Read failed verifications from REPORT.md
    2. Spawn executor with fix instructions:
       Task(
         prompt="Fix visual verification failures.
         
         ## Failed Verifications
         {extracted failures from REPORT.md}
         
         ## Instructions
         - Review the failed elements
         - Ensure data-testid attributes are present
         - Fix any styling/visibility issues
         - Commit fixes
         
         Return when fixes are committed.",
         subagent_type="gsd-executor",
         model="{executor_model}"
       )
    
    3. Re-run visual verifier
    4. retry_count++
```

**If still FAILED after max_retries:**
```markdown
⚠️ Visual verification incomplete after {max_retries} retries.

**Failed verifications:**
{list from REPORT.md}

Report saved: {PHASE_DIR}/verification/REPORT.md

Continuing to phase verification. Review manually before considering phase complete.
```

Add to `pending_visual_issues` list for final report.

**Cleanup on PASSED:**
```bash
# Remove screenshots folder (not needed if all passed)
rm -rf "$PHASE_DIR/verification/screenshots"
# Keep REPORT.md as record
```
...
**Check for pending visual issues:**

If `pending_visual_issues` is not empty:
```markdown
---

⚠️ **Visual Verification Incomplete**

The following visual checks could not be verified automatically:

{list pending_visual_issues}

Reports saved in: `.planning/phases/{phase}/verification/`

Please review manually before considering phase fully complete.
