...
When `visual_verification.enabled` is true in `.planning/config.json`, identify tasks that require browser-based verification.

**Visual task indicators:**
- Modifies files in `src/components/`, `src/app/`, `app/`, `pages/`
- Creates or modifies `.tsx` files with UI elements
- Task description mentions: layout, styling, display, render, show, visible, button, form, modal, header, footer, navigation
- File paths include: `page.tsx`, `layout.tsx`, component names

**For each visual task, create a verification entry:**

```markdown
### N. [What the user should see]
- **URL:** [Route where change is visible]
- **Viewport:** desktop|mobile (based on component type)
- **Element:** [data-testid selector - MUST be added by executor]
- **Expected:** [Observable state - visible, contains text, has class, etc.]
- **Context:** Task N [brief description of what was changed]
```

**Example visual verifications:**

```markdown
<visual_verifications>

### 1. Header displays user avatar
- **URL:** /dashboard
- **Viewport:** desktop
- **Element:** [data-testid="header-user-avatar"]
- **Expected:** Avatar image visible, circular shape
- **Context:** Task 2 added UserAvatar component to header

### 2. Orders table shows Status column
- **URL:** /orders
- **Viewport:** desktop
- **Element:** [data-testid="orders-table"] th:has-text("Status")
- **Expected:** Column header visible, sortable
- **Context:** Task 4 added status column to OrdersTable

### 3. Mobile navigation menu works
- **URL:** /
- **Viewport:** mobile
- **Element:** [data-testid="mobile-nav-menu"]
- **Expected:** Menu opens on hamburger click, shows nav items
- **Context:** Task 5 implemented responsive navigation

</visual_verifications>
```

**IMPORTANT:** When creating visual verifications, ensure:
1. The executor knows to add `data-testid` attributes (reference CONVENTIONS.md)
2. The URL is a real route in the application
3. The expected state is objectively verifiable (not subjective like "looks good")
4. Mobile verifications use `viewport: mobile`

**If NO visual tasks in plan:** Omit the `<visual_verifications>` section entirely
...
<visual_verifications>
<!-- Only include if visual_verification.enabled in config AND plan has UI tasks -->

### 1. [Description of what to verify]
- **URL:** /path/to/page
- **Viewport:** desktop|mobile
- **Element:** [data-testid="element-id"]
- **Expected:** Element visible, correct text/state
- **Context:** Task N added/modified this element
</visual_verifications>
